...
read_highwater() {
  ...
  hw_last_session_id=$(jq -r '._last.session_id // "" | tostring' "$HIGHWATER_FILE" 2>/dev/null)
  hw_last_api_ms=$(jq -r '._last.api_duration_ms // 0 | tonumber? // 0 | floor | tostring' "$HIGHWATER_FILE" 2>/dev/null)
  hw_last_output_tokens=$(jq -r '._last.output_tokens // 0 | tonumber? // 0 | floor | tostring' "$HIGHWATER_FILE" 2>/dev/null)
}
